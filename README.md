...
## 一、插件简介
### 1.1 插件名称
`shareholder_feedback_monitor`（股东回馈公告监控插件）
...
## 二、安装与加载
### 2.1 安装步骤
1. 将插件代码保存为文件 `shareholder_feedback_monitor.py`
2. 复制文件到 `astrbot` 插件目录（通常为 `plugins/` 文件夹）
3. 重启 `astrbot` 机器人，插件会自动加载

### 2.2 加载验证
重启后查看机器人日志，若出现以下信息则说明加载成功：
```
股东回馈公告监控插件已加载，准备启动定时任务。
```
...
### 5.1 插件加载失败
- 检查Python版本是否为3.8+
- 确认插件文件名称正确（`shareholder_feedback_monitor.py`）
...
### 5.3 定时推送无消息
- 检查是否已设置通知群组（管理员执行`/set_notify_group`确认）
